...
_CALLBACK_SIGNATURES: Dict[str, Tuple[Tuple[str, str], ...]] = {
    "before_model": (
    ...
    "before_agent": (("callback_context", "CallbackContext"),),
    "after_agent": (("callback_context", "CallbackContext"),),
    "before_tool": (
        ("tool", "Tool"),
        ("input", "dict[str, Any]"),
        ("callback_context", "CallbackContext"),
...
_CALLBACK_RETURNS: Dict[str, str] = {
    "before_model": "Optional[LlmResponse]",
    "after_model": "Optional[LlmResponse]",
    "before_agent": "Optional[Content]",
    "after_agent": "Optional[Content]",
    "before_tool": "Optional[dict[str, Any]]",
    "after_tool": "Optional[dict[str, Any]]",
}
...
def _callback_file(
    ...
    content = _callback_code(callback_type, channel, target_agent)
    # The directory name is checked here for maintainers, while the adapter
    # card references only the project-relative pythonCode path.
...
def _callback_code(
    callback_type: str, channel: str, target_agent: str
) -> str:
    function_name = _CALLBACK_FUNCTIONS[callback_type]
    params = _CALLBACK_SIGNATURES[callback_type]
    return_type = _CALLBACK_RETURNS[callback_type]
    signature = "\n".join(
        [
            f"def {function_name}(",
            *[f"    {name}: {annotation}," for name, annotation in params],
            f") -> {return_type}:",
        ]
    )
    return (
        ...
        f"{signature}\n"
        f"    \"\"\"Run starter {channel} behavior for {target_agent}.\"\"\"\n"
        f"{_callback_body(callback_type, channel)}"
    )


def _callback_body(callback_type: str, channel: str) -> str:
    if callback_type == "before_model":
        return (
            "    hint = (\n"
            f'        "Starter {channel} channel hint. Replace this "\n'
            '        "callback with real channel-specific behavior, or "\n'
            '        "delete it if static instruction diffs are enough."\n'
            "    )\n"
            "    try:\n"
